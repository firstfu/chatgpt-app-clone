...
export default function ModelSelect() {
  ...
  return (
    <div className="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-xl">
      {models.map(item => {
        let selected = item.id === state.currentModel;
        return (
          <button
            key={item.id}
          ...
          >
            <span className={`  group-hover:text-[#26cf83] transition-colors duration-75   ${selected ? "text-[#26cf83]" : ""}`}>
              <item.icon />
            </span>
            <span className="transition-colors duration-75">{item.name}</span>
          </button>
        );
      })}
    </div>
